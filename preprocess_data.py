...
def preprocess_data(raw_df: pd.DataFrame, target_col: str, scaler_numeric: bool = True) -> Dict[str, Any]:
    ...
    """
    raw_df[target_col] = raw_df[target_col].map({'no': 0, 'yes': 1})
    split_dfs = split_data(raw_df, target_col)
    input_cols = [col for col in raw_df.columns if col not in [target_col, 'nr.employed', 'emp.var.rate', 'duration']]
...
def preprocess_new_data(
    ...
    for col in ['nr.employed', 'emp.var.rate', 'duration']:
